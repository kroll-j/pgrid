...
window_list *grplist_find(window_list *start, char *name)
{
  window_list *wnd= start;
  prim_group *pg;
...
dword clone_group(char *name, dword hparent, int x, int y,
...
dword clone_frame(char *name, dword hparent)
{
 ...
 *
 */
int wnd_addprop(dword id, char *name, prop_t value, int type, char *desc)
{
 ...
 *
 */
prop_t wnd_getprop(dword id, char *name)
{
  window_list *wnd= wlist_find(windows, id);
  if(!wnd) wnd= wlist_find(groups, id);
  if(!wnd) { /*logmsg("prop_get: %d not found\n", id);*/ return 0; }

  primitive *prim= wnd->self;
  property *prop;

  if(!prim->props) { /*logmsg("%d: no props\n", id);*/ return 0; }
 ...
 *
 */
prop_t wnd_setprop(dword id, char *name, prop_t value)
{
  window_list *wnd= wlist_find(windows, id);
  if(!wnd) wnd= wlist_find(groups, id);
  if(!wnd) return 0;
...
int wnd_getproptype(dword id, char *name)
{
  window_list *wnd= wlist_find(windows, id);
  if(!wnd) wnd= wlist_find(groups, id);
  if(!wnd) return 0;
...
bool text_settext(dword id, char *text)
{
  window_list *wnd= wlist_find(windows, id);
  if(!wnd) wnd= wlist_find(groups, id);
  if(!wnd) return false;
...
void wnd_forceredraw(dword id, rect *rc)
{
  ...
  if(rc)
  {
    rc->x+= pos.x; rc->y+= pos.y;
    rc->rgt+= pos.x; rc->btm+= pos.y;
    clip_rect(&pos, rc);
  }
...
window_list *prim_create_text(window_list *prev, bool is_child,
                              ...
                              char *text, dword col, font *fnt, int align)
{
...
dword create_text(dword parent, int x,int y, int width,int height, char *text, int col, font *fnt, int align)
{
  window_list *prev;
  window_list *wnd;
  bool redraw;
...
void frame_setcolors(dword id, dword col_left, dword col_top, dword col_right, dword col_btm)
{
  ...
  self->col_left= col_left; self->col_top= col_top;
  self->col_right= col_right; self->col_btm= col_btm;
...
#ifndef REDRAW_EVERY_FRAME
  redraw_rect(&rcl); redraw_rect(&rct); redraw_rect(&rcr); redraw_rect(&rcb);
  update_rect(&rcl); update_rect(&rct); update_rect(&rcr); update_rect(&rcb);
#endif
}
...
{
    if(p->scanline_cache) free(p->scanline_cache); p->scanline_cache= 0;
    if(p->outline_cache) free(p->outline_cache); p->outline_cache= 0;
...
{
    if( !parent && !(parent= find_prim_pos(x, y, true)) ) { /*logmsg("no widget\n");*/ return 0; }
...
{
  if(!rc || !clp)
  { logmsg("clip_rect(%d, %d)!!\n", rc, clp); video_cleanup(); exit(1); }
 ...
 * upd: das Rechteck
 */
void prim_redraw(primitive *p, rect *_abspos, rect *upd)
{
...
//void blt_rgba(rgba *_src, int src_w, int src_h, rect rc, int dst_x, int dst_y)
      ...
      rect rc= *upd;
      rc.x-= abspos.x; rc.rgt-= abspos.x;
      rc.y-= abspos.y; rc.btm-= abspos.y;
...
void redraw_children(rect *prntrc, window_list *wnd, const rect *rc)
{
  ...
  if( (child= wnd->self->children) ) do
  {
    ...
    rect prc= { prntrc->x,prntrc->y, prntrc->rgt,prntrc->btm };
...
void recurse_redraw_rect(rect *rc, window_list *wnd)
{
...
// @!
    ...
    if(self->flags.visible & (gui_active | self->flags.sticky) & clip_rect(&clp, &abspos))
    {
      ...
      if(!self->flags.transparent)
      {
        int i;
        rect rcs[4];
        if( (i= subtract_rect(upd, clp, rcs)) )
        {
          for(i--; i>=0; i--)
            if(wnd->prev) recurse_redraw_rect(&rcs[i], wnd->prev);
        }
      }
...
void dump_wlist(window_list *src, int indent= 0)
{
  ...
  do
  {
    ...
    if(lst->self->props) { INDENTLOG("props: %08X\n", lst->self->props); }
    ...
    { INDENTLOG("cb_mouse: %08X\n", lst->self->callbacks.mouse); }

    INDENTLOG("this: %08X\n", lst);
    INDENTLOG("prev: %08X\n", lst->prev);
    INDENTLOG("next: %08X\n", lst->next);
    INDENTLOG("parent: %08X\n", lst->self->parent);
    fflush(stdout);
...
void flux_mouse_event(double x, double y, int btn)
{
    ...
    mouse.x= x; mouse.y= y; mouse.btns= btn;
    ...
    oldmouse.x= x; oldmouse.y= y; oldmouse.btns= btn;
}
